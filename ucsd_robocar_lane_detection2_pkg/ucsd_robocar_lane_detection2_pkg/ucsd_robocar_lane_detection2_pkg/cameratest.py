...
# Get argument first
nnPath = str((Path(__file__).parent / Path('depthai-python/examples/models/mobilenet-ssd_openvino_2021.4_6shave.blob')).resolve().absolute())
if len(sys.argv) > 1:
    nnPath = sys.argv[1]
...
with dai.Device(pipeline) as device:
    ...
    def displayFrame(name, frame):
        for detection in detections:
            ...
            if(labelMap[detection.label] == "person"): print(1)
        if not nnRegion:
